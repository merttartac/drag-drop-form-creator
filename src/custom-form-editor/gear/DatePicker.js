...
export default class MyDatePicker extends Component {
	...
	constructor(props) {
		...
		this.state = {
			...
			backwardYearArrowRef: React.createRef(),
			forwardYearArrowRef: React.createRef(),
			backwardMonthArrowRef: React.createRef(),
			forwardMonthArrowRef: React.createRef()
		}
	}
	...
	componentDidMount() {
		...
		this.props.value && this.setDateToInput(this.props.value)
	}
	...
	addBackDrop = e => {
		...
		if (this.state.showDatePicker && !this.state.inputRef.current.contains(e.target) &&
				!this.state.backwardYearArrowRef.current.contains(e.target) &&
				!this.state.forwardYearArrowRef.current.contains(e.target) &&
				!this.state.backwardMonthArrowRef.current.contains(e.target) &&
				!this.state.forwardMonthArrowRef.current.contains(e.target)) {
		...
		return (
				<div className='MyDatePicker'>
					<div className='mdp-input' onClick={() => this.showDatePicker(true)}>
						<input type='date' onChange={this.updateDateFromInput} ref={this.state.inputRef}/>
					</div>
					{this.state.showDatePicker ? (
							<div className='mdp-container'>
								<div className='mdpc-head'>
									<div className='mdpch-button'>
										<div ref={this.state.backwardYearArrowRef} className='mdpchb-inner'
											 onClick={() => this.setYear(-1)}>
											<span className='mdpchbi-left-arrows'></span>
										</div>
									</div>
									<div className='mdpch-button'>
										<div ref={this.state.backwardMonthArrowRef} className='mdpchb-inner'
									...
									<div className='mdpch-button'>
										<div ref={this.state.forwardMonthArrowRef} className='mdpchb-inner'
											 onClick={() => this.setMonth(1)}>
											<span className='mdpchbi-right-arrow'></span>
										</div>
									</div>
									<div ref={this.state.forwardYearArrowRef} className='mdpch-button'
										 onClick={() => this.setYear(1)}>
